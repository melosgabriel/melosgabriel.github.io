...
int main(int argc, char* argv[]){
	...
	Mat sudoku = imread(argv[1],CV_LOAD_IMAGE_COLOR);
	...
	vector<Vec2f> lines; 
	Canny(sudoku_grid,sudoku_grid,100,300,3);

	HoughLines(sudoku_grid, lines, 1, CV_PI/180, 120);
	...
	for( size_t i = 0; i < lines.size(); i++ )
{
  ...
  line( sudoku, pt1, pt2, Scalar(134,14,203), 2, CV_AA);
}

	imshow("lines",sudoku); waitKey(0);
	...
	Mat undistortedThreshed = sudoku_undistorted.clone(); 
	adaptiveThreshold(sudoku_undistorted, undistortedThreshed, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY, 101, 1);
	...
	imshow("sudoku_grid",sudoku_grid);
	imshow("sudoku_undistorted.png",undistortedThreshed);
	waitKey(0);
}
