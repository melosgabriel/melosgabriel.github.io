...
int main(int argc, char* argv[]){
 ...
 if(contours_poly[0].size()==4){
    ...
    line(sudoku_original,P1,P2, Scalar(0,0,255),1,CV_AA,0);
    line(sudoku_original,P2,P3, Scalar(0,0,255),1,CV_AA,0);
    line(sudoku_original,P3,P4, Scalar(0,0,255),1,CV_AA,0);
    line(sudoku_original,P4,P1, Scalar(0,0,255),1,CV_AA,0);
    rectangle(sudoku_original,boundRect,Scalar(0,255,0),1,8,0);
    //rectangle(transformed,boundRect,Scalar(0,255,0),1,8,0);
   
	GaussianBlur(sudoku_undistorted_gray,sudoku_undistorted_gray,Size(21,21),0);
   	adaptiveThreshold(sudoku_undistorted_gray,sudoku_undistorted_gray, 255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 15, 2);
   	imshow("crote",sudoku_undistorted_gray); waitKey(0);
    ...
    for(int i = 0;i<9;i++){
    	for(int j = 0;j<9;j++){
	    	...
	    	putText(sudoku_undistorted, to_string(values_solved.at<uchar>(j,i)), 
	    		Point(i*sudoku_undistorted.rows/9 + 10,(j+1)*sudoku_undistorted.cols/9 - 10),
	    		FONT_HERSHEY_SIMPLEX, 8.1, Scalar (255,0,0),5);
    	}
    }
